...
* あとがき
  ...
  Fcitxではやり方がわかりませんでした。[[https://github.com/ch11ng/exim/][exim]] はmozcで漢字変換ができず (helperを呼ぶように書き換える必要がありそう)，[[https://github.com/zachcurry/emacs-anywhere][emacs-anywhere]] はフロートウインドウを出そうとして拒否されるのでEXWM用に書き換えないと動かないと思います。
